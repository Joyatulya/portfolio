...
class InsulinAnalysis {
	...
	filter_hypos = (data: PARSED_BM[]): PARSED_BM[] => {
		return data.filter(this.is_hypo)
	}

	bm_aggregation(data: PARSED_BM[], reading_type: bmType, days: number = 4, check_reading_num = true) {
		/**
		...
		const specifid_bms_specific_days = specified_days.filter(x => x.type === reading_type)
		...
		const mean_bm = specifid_bms_specific_days.reduce((a, c) => a + c.value, 0) / specifid_bms_specific_days.length
